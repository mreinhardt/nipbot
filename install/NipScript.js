...
function base() {
    "undefined" == typeof nxScript || 0 === nxScript ? (AutoWoot = 1, AutoList = AutoCurate = 1, nxButtons = 0, ListSort = ListPos = 1, AFKTmr = 1, API.off(), API.on(API.ADVANCE, nx.OnAdvance), API.on(API.VOTE_UPDATE, nx.DoMeh), API.on(API.GRAB_UPDATE, nx.OnCurate), API.on(API.USER_JOIN, nx.OnJoin), API.on(API.USER_LEAVE, nx.OnLeave), API.on(API.CHAT, nx.OnChat), API.on(API.WAIT_LIST_UPDATE, nx.OnWlUpdate), console.log(notice), nx.GetSettings(), $("#chat-messages").append('<div class="message welcome nxnotif"><span class="text">' + notice + '</span></div>'), $(window).bind('beforeunload', function(){ return "You about to navigate away from this plug.dj community!" }), $(".nxnotif").css("color", "#89be6c"), $("#chat-messages").scrollTop($("#chat-messages div.message").last().position().top - $("#chat-messages div.message").first().position().top + 100), 1 == AutoWoot && nx.Woot(), 1 == AutoList && nx.CreateUserlist(), nx.initial(), nx.initAFK(), nx.showAFK(), nxScript = 1, $(window).on("focus", nx.OnFocus), $("#app").on("mousedown", nx.DoButton), $("#users-button").on("click", nx.DoMeh), $(".button.room").on("click", nx.DoMeh), $("#user-lists .list.room .nxuser .icon-meh").css({
        left: "auto",
        right: "9px",
        top: "-1px"
